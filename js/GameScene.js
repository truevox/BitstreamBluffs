...
export default class GameScene extends Phaser.Scene {
    ...
    update(time, delta) {
        ...
        if (this.manette.isWalkMode()) {
            ...
            // Initialize walk animation state if not already set
            if (this.walkAnimTime === undefined) {
                this.walkAnimTime = 0;
                this.playerOriginalRotation = this.player.rotation;
                console.log('Initializing walk animation, storing original rotation:', this.playerOriginalRotation);
            }
            ...
            // Check if we're walking in either direction
            const isWalking = this.manette.isActionActive('walkLeft') || 
                              this.manette.isActionActive('walkRight') || 
                              this.cursors.left.isDown || 
                              this.cursors.right.isDown;
            
            // Move left with A key or left stick or cursor left
            if (this.manette.isActionActive('walkLeft') || this.cursors.left.isDown) {
                Body.translate(this.player.body, { x: -walkSpeed, y: 0 });
            }
            
            // Move right with D key or right stick or cursor right
            if (this.manette.isActionActive('walkRight') || this.cursors.right.isDown) {
                Body.translate(this.player.body, { x: walkSpeed, y: 0 });
            }
            
            // Apply oscillating walking animation when moving
            if (isWalking) {
                // Update animation timer
                this.walkAnimTime += 0.1; // Increment animation time
                
                // Calculate oscillating rotation using sine wave
                // Hardcode some reasonable values if PhysicsConfig.walkMode is undefined
                const walkAnimSpeed = 10; // Animation frequency - lower is faster
                const walkAnimRotation = 15; // Max tilt angle in degrees
                
                const angleInRadians = Math.sin(this.walkAnimTime * Math.PI / walkAnimSpeed) 
                                    * (walkAnimRotation * Math.PI / 180);
                
                // Apply rotation to player sprite and its physics body
                this.player.rotation = (this.playerOriginalRotation || 0) + angleInRadians;
                Body.setAngle(this.player.body, this.player.rotation);
                
                console.log(`Walk animation: angle=${(angleInRadians * 180 / Math.PI).toFixed(2)}°`);
            }
            
            // Make the player upright when not walking
            if (!isWalking) {
                // Reset to original rotation
                if (this.playerOriginalRotation !== undefined) {
        ...
        if (!this.onGround) {
            ...
            // Use a lower air rotation velocity to prevent physics issues
            const safeAirRotVel = Math.min(airRotVel, 0.05); // Cap rotation speed to prevent physics glitches
            
            // W key/left-stick up for counter-clockwise rotation in air
            if (this.manette.isActionActive('rotateCounterClockwise')) {
                // Apply the rotation directly rather than through angular velocity
                // This is more stable for physics simulations
                this.player.rotation -= safeAirRotVel;
                Body.setAngle(this.player.body, this.player.rotation);
                deltaRotation = -Phaser.Math.RadToDeg(safeAirRotVel);
            }
            ...
            // No need for an else case - we'll preserve current rotation
...
// Add explosion effect on crash using modern Phaser 3.60+ particle system
GameScene.prototype.createExplosionEffect = function(x, y) {
    try {
        // Create a simple visual explosion with circles instead of particles
        const explosionGroup = this.add.group();
        const colors = [0xffff00, 0xff0000, 0xff5500, 0xffffff]; // Yellow, red, orange, white
        
        // Create 15 particles manually
        for (let i = 0; i < 15; i++) {
        ...
        // Auto-destroy the group after it's done
        this.time.delayedCall(1100, () => {
            explosionGroup.destroy(true);
        });
    } catch (e) {
        console.warn('Error creating explosion effect:', e);
    }
};
