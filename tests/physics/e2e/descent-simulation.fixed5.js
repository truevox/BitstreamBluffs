...
class DescentSimulator {
  ...
  forceGrounded() {
    this.player.onGround = true;
    this.player.velocity.y = 0;
    // Must record state after changing properties
    this.recordState();
    return true;
  }
...
describe('Sled Physics E2E Descent Simulation', () => {
  ...
  test('jumping adds vertical velocity and air time', measurePerformance(() => {
    ...
    simulator.recordState(); // Record initial state
    ...
    // Skip directly to verifying that jumping worked correctly
    expect(simulator.stateHistory.some(state => state.velocity.y < 0)).toBe(true);
    expect(simulator.stateHistory.some(state => !state.onGround)).toBe(true);
  }));
